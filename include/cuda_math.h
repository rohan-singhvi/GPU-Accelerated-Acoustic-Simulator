...
HD float3 operator*(const float3& a, float s) { return make_float3(a.x * s, a.y * s, a.z * s); }

HD float3 operator*(float s, const float3& a) { return make_float3(a.x * s, a.y * s, a.z * s); }

HD float dot(const float3& a, const float3& b) { return a.x * b.x + a.y * b.y + a.z * b.z; }

HD float length(const float3& a) { return sqrtf(dot(a, a)); }

HD float length_sq(const float3& a) { return dot(a, a); }
