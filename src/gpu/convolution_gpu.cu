...
std::vector<float> apply_reverb_gpu(const std::vector<float>& dry_signal,
                                    const std::vector<float>& impulse_response, float mix) {
    ...
    std::cout << "FFT Size: " << fft_size << " (Dry: " << n_dry << ", IR: " << n_ir << ")"
              << std::endl;
