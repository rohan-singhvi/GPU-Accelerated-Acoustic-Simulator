...
inline float intersect_triangle_cpu(const float3& ray_origin, const float3& ray_dir,
                                    const float3& v0, const float3& v1, const float3& v2,
                                    const float3& tri_normal, float3& out_normal) {
...
void run_simulation_cpu(const SimulationParams& params, const MeshData& mesh,
                        std::vector<float>& ir) {
    ...
    std::cout << "Running CPU Simulation (OpenMP Threads: " << omp_get_max_threads() << ")..."
              << std::endl;
        ...
        for (int bounce = 0; bounce < 50; ++bounce) {
            ...
            if (params.room_type == SHOEBOX) {
                ...
                for (int t = 0; t < mesh.num_triangles; ++t) {
                    float dist = intersect_triangle_cpu(px, dx, mesh.v0[t], mesh.v1[t], mesh.v2[t],
                                                        mesh.normals[t], temp_n);
                    if (dist < min_dist) {
                        min_dist = dist;
                        nx = temp_n;
                        hit_tri_idx = t;
            ...
            if (roll < params.wall_transmission) {
                ...
                // We push the origin slightly past the wall to avoid self-intersecting the same
                // spot
